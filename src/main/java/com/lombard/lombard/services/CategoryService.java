...
public class CategoryService {
    ...
    // DODANA NOWA METODA DO KASKADOWEGO USUWANIA KATEGORII
    @Transactional
    public boolean deleteCategoryWithCascade(Integer id) {
        System.out.println("Trying to delete category with cascade, ID: " + id);
        return categoryRepository.findById(id)
                .map(category -> {
                    try {
                        // Najpierw rekurencyjnie usuń wszystkie podkategorie
                        if (category.getSubcategories() != null && !category.getSubcategories().isEmpty()) {
                            List<Category> subcategories = category.getSubcategories();
                            System.out.println("Deleting " + subcategories.size() + " subcategories");

                            // Iterujemy po kopii listy, aby uniknąć ConcurrentModificationException
                            for (Category subcategory : List.copyOf(subcategories)) {
                                deleteCategoryWithCascade(subcategory.getId());
                            }
                        }

                        // Sprawdź czy kategoria ma powiązane przedmioty - nadal nie usuwamy kategorii z przedmiotami
                        if (category.getItems() != null && !category.getItems().isEmpty()) {
                            System.out.println("Category has " + category.getItems().size() + " items, cannot delete");
                            return false;
                        }

                        categoryRepository.delete(category);
                        System.out.println("Category deleted successfully with cascade");
                        return true;
                    } catch (Exception e) {
                        System.err.println("Error deleting category with cascade: " + e.getMessage());
                        e.printStackTrace();
                        return false;
                    }
                })
